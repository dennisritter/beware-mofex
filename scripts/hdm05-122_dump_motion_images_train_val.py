...
def load_seqs_asf_amc_hdm05(root, regex_str_asf, regex_str_amc):
    ...
    for amc_path in Path(root).rglob(regex_str_amc):
        ...
        seqs.append({'asf': asf_path, 'amc': str(amc_path), 'name': amc_file, 'desc': class_dir.split('/')[-1]})
...
for seq_dict in seqs:
    seq = seq_loader.load(seq_dict['asf'], seq_dict['amc'], seq_dict['name'], seq_dict['desc'])
    ...
    seq.positions = normalizations.pose_position(seq.positions, (seq.positions[:, LEFT_IDX, :] + seq.positions[:, RIGHT_IDX, :]) * 0.5)
...
iqr_factor_y = 2.5
iqr_factor_z = 3.5
minmax_xyz = xyz_minmax_coords(seqs_chunked, [iqr_factor_x, iqr_factor_y, iqr_factor_z], plot_histogram=False)
...
for label in chunks_labeled_dict.keys():
    label_split = train_test_split(chunks_labeled_dict[label], test_size=0.1, random_state=42)
