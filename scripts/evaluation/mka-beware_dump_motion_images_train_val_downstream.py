...
dataset_name = 'mka-beware-1.1_cookie-3.0'
...
seq_labeled_dict = {}
for filename in sorted(Path(src_root).rglob('*.json')):
    ...
    print(f'Loaded: {seq.name} [{seq.desc}] - len {len(seq)}')
...
for seq_class in seq_labeled_dict:
    for i, seq in enumerate(seq_labeled_dict[seq_class]):
        ...
        _seq_append = _seq[:]
        _seq_append.name = f'{_seq.name}_l{len(_seq_append)-1}'
        ...
        for idx, j in enumerate([1]):
            for k in range(len(seq)):
                ...
                if rep == 'rep':
                    ...
                    #         chunks_labeled_dict[rep].append(_seq_append)
                    _seq_append = _seq[5:]
                    _seq_append.name = f'{_seq.name}_l{len(_seq_append)-1}_f{5}'
                    _seq_append.desc = rep
                    ...
                    for m in range(1, 4):
                        _seq_append = _seq[::m]
                        _seq_append.name = f'{_seq.name}_l{len(_seq_append)-1}_s{m}'
                        _seq_append.desc = rep

                        seqs_chunked.append(_seq_append)
                        chunks_labeled_dict[rep].append(_seq_append)

# remove every 2nd step of noreps
print(f'len of rep: {len(chunks_labeled_dict["rep"])}')
print(f'len of norep: {len(chunks_labeled_dict["norep"])}')
chunks_labeled_dict['norep'] = chunks_labeled_dict['norep'][::2]
print(f'len of reduced norep: {len(chunks_labeled_dict["norep"])}')
